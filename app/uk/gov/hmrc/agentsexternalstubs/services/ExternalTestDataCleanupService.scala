...
import uk.gov.hmrc.agentsexternalstubs.connectors.{AgentPermissionsConnector, AgentUserClientDetailsConnector}
...
class ExternalTestDataCleanupService @Inject() (
  ...
  def deleteTestData(arn: String, groupId: String): Future[Unit] =
    for {
      _ <- aucdConnector.deleteTestData(arn, groupId)
      _ <- apConnector.deleteTestData(arn)
    } yield Future.unit
}
