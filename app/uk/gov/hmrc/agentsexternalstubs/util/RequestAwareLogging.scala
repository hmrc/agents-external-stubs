...
import uk.gov.hmrc.agentsexternalstubs.util.RequestSupport.given
...
class RequestAwareLogger(
  ...
  private def hc(using request: RequestHeader): HeaderCarrier = summon[HeaderCarrier]

  private def sessionId(using request: RequestHeader) = s"[SessionId: ${hc.sessionId.getOrElse("")}]"

  private def requestId(using request: RequestHeader) = s"[RequestId: ${hc.requestId.getOrElse("")}]"

  private def referer(using r: RequestHeader) = s"[Referer: ${r.headers.get(HeaderNames.REFERER).getOrElse("")}]"

  private def userAgent(using r: RequestHeader) =
    s"[UserAgent: ${r.headers.get(HeaderNames.USER_AGENT).getOrElse("")}]"

  private def deviceId(using r: RequestHeader) = s"[DeviceId: ${hc.deviceID}]"
