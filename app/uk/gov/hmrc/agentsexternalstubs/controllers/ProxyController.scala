...
@Singleton
class ProxyController @Inject() (ws: WSClient, config: Configuration, cc: ControllerComponents)(using
  ...
  def proxyPassTo(url: String): Action[AnyContent] = Action.async { request =>
    given Request[AnyContent] = request
    targetUrlFor(url) match {
      case Some(targetUrl) =>
        ...
        val upstreamRequest = ws
          ...
          .withQueryStringParameters(request.queryString.toSeq.flatMap { case (k, sv) => sv.map(v => (k, v)) }*)
          .withHttpHeaders(hc.withExtraHeaders(requestContentType*).headersForUrl(Config())(targetUrl)*)
        ...
        upstreamRequest
          ...
          .map { response =>
            ...
            Results
              ...
              .withHeaders(response.headers.toSeq.flatMap { case (k, sv) => sv.map(v => (k, v)) }*)
          }
      case None =>
        Future.successful(
  ...
  def targetUrlFor(path: String): Option[String] =
    path.split("/").filterNot(_.isEmpty).headOption.flatMap { prefix =>
      Try(config.underlying.getConfig(s"microservice.services.$prefix")).toOption.flatMap { sc =>
        ...
        } yield s"$protocol://$host:$port${if path.startsWith("/") then path else s"/$path"}"
      }
    }
