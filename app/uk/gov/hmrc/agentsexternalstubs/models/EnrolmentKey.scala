...
object EnrolmentKey {
  ...
  def parse(s: String): Either[String, EnrolmentKey] = {
    val parts = s.split("~")
    if parts.nonEmpty && parts.size >= 3 && parts.size % 2 == 1 then
  ...
  def validateService(ek: EnrolmentKey): Either[String, EnrolmentKey] =
    if ek.service.nonEmpty then Services(ek.service).map(_ => Right(ek)).getOrElse(Left("INVALID_SERVICE"))
  ...
  def validateIdentifiers(ek: EnrolmentKey): Either[String, EnrolmentKey] = Services(ek.service) match {
    ...
    case Some(service) =>
      ek.identifiers
        ...
        .flatMap(_ => if ek.identifiers == ek.identifiers.sorted then Right(ek) else Left("INVALID_IDENTIFIERS"))
  }
  ...
  val tolerantReads: Reads[EnrolmentKey] = new Reads[EnrolmentKey] {
    override def reads(json: JsValue): JsResult[EnrolmentKey] = json match {
      case JsString(value) =>
        ...
        if parts.headOption.exists(_.nonEmpty) && parts.tail.forall(_.isEmpty) then
