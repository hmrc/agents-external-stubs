...
import uk.gov.hmrc.agentsexternalstubs.support.{MongoApp, ServerBaseISpec, TestRequests}

class SignInControllerISpec extends ServerBaseISpec with MongoApp with TestRequests {
  ...
  "SignInController" when {
    ...
    "GET /agents-external-stubs/session" should {
      "return session data" in {
        val result1 = ThisApp.signIn("foo123", "boo")
        result1.status shouldBe 201
        val result2 = ThisApp.authSessionFor(result1)
